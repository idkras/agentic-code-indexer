...
# Add the agentic_code_indexer directory to Python path to find common_data_format
current_dir = Path(__file__).parent
agentic_code_indexer_dir = current_dir.parent / "agentic_code_indexer"
sys.path.insert(0, str(agentic_code_indexer_dir))
...
class PythonASTVisitor(cst.CSTVisitor):
    ...
    def visit_ClassDef(self, node: cst.ClassDef) -> None:
        ...
        class_node = ClassNode(
            ...
            raw_code=(
                self.source_code[
                    node.metadata[metadata.PositionProvider].start.line
                    - 1: node.metadata[metadata.PositionProvider].end.line
                ]
        ...
        self.chunker.relationships.append(
            Relationship(source_id=self.file_node.id, target_id=class_node.id, type=RelationshipType.CONTAINS)
        )
    ...
    def visit_FunctionDef(self, node: cst.FunctionDef) -> None:
        ...
        if is_method:
            ...
            self.chunker.relationships.append(
                Relationship(source_id=self.current_class.id, target_id=func_node.id, type=RelationshipType.DEFINES)
            )
        else:
            ...
            self.chunker.relationships.append(
                Relationship(source_id=self.file_node.id, target_id=func_node.id, type=RelationshipType.CONTAINS)
            )
        ...
        if node.params:
            for param in node.params.params:
                if isinstance(param.name, cst.Name):
                    ...
                    self.chunker.relationships.append(
                        Relationship(source_id=func_node.id, target_id=param_node.id, type=RelationshipType.DECLARES)
                    )
    ...
    def visit_ImportFrom(self, node: cst.ImportFrom) -> None:
        ...
        self.chunker.relationships.append(
            Relationship(source_id=self.file_node.id, target_id=import_node.id, type=RelationshipType.IMPORTS)
        )
...
def main():
    ...
    logger.info(
        f"Processed {len(chunker.processed_files)} files, extracted {len(chunker.nodes)} nodes, {len(chunker.relationships)} relationships"
    )
