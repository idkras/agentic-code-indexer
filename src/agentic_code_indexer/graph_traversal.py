...
class GraphTraversalEngine:
    ...
    def _init_traversal_rules(self):
        ...
        self.traversal_rules = {
            ...
            "Class": [
                TraversalRule("Class", "CONTAINS", direction=TraversalDirection.INCOMING),
                # File that contains this class
                ...
                TraversalRule("Class", "IMPLEMENTS", direction=TraversalDirection.INCOMING),
                # Classes that implement this
                TraversalRule("Class", "INSTANTIATES", direction=TraversalDirection.INCOMING),
                # Methods that create instances
            ],
            ...
            "Method": [
                TraversalRule("Method", "DEFINES", direction=TraversalDirection.INCOMING),
            ...
            "Function": [
                TraversalRule("Function", "CONTAINS", direction=TraversalDirection.INCOMING),
            ...
            "Variable": [
                TraversalRule("Variable", "DECLARES", direction=TraversalDirection.INCOMING),
                # Method/function that declares
    ...
    async def expand_context(
        ...
        traversal_summary = self._create_traversal_summary(search_results, all_related_nodes, all_relationships)
    ...
    async def _traverse_from_node(
        ...
        for rule in rules[node_type]:
            ...
            nodes, rels = await self._apply_traversal_rule(
                node_id, rule, max_nodes - len(related_nodes), include_source_code, processed_ids
            )
    ...
    async def _apply_traversal_rule(
        ...
        if rule.max_depth == 1:
            ...
            path_pattern = (
                f"(start)-{direction_clause}[r:{rule.relationship_type}*1..{rule.max_depth}]->(related{type_filter})"
            )
        ...
        with self.driver.session() as session:
            try:
                result = session.run(query, start_id=start_node_id, limit=max_nodes)
    ...
    def _create_traversal_summary(
        self, central_nodes: List[SearchResult], related_nodes: List[GraphNode], relationships: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def get_call_hierarchy(
        self, method_id: str, direction: TraversalDirection = TraversalDirection.BOTH, max_depth: int = 3
...
async def main():
    ...
    try:
        ...
        call_hierarchy = await traversal_engine.get_call_hierarchy(
            "method_123", direction=TraversalDirection.BOTH, max_depth=2
        )
