...
class VectorSearchEngine:
    ...
    async def search_by_text(
        self, query_text: str, config: VectorSearchConfig = None, node_types: Optional[List[str]] = None
    ) -> List[SearchResult]:
        """
        ...
        for node_type in node_types:
            ...
            results = await self._search_node_type(node_type, query_embedding, query_text, config)
    ...
    async def _search_node_type(
        self, node_type: str, query_embedding: List[float], query_text: str, config: VectorSearchConfig
    ) -> List[SearchResult]:
    ...
    async def search_similar_to_node(
        self, node_id: str, config: VectorSearchConfig = None, exclude_self: bool = True
    ) -> List[SearchResult]:
        """
        ...
        similar_results = await self._search_node_type(reference_type, reference_embedding, "", config)
    ...
    async def search_by_embedding(
        self, embedding: List[float], node_types: Optional[List[str]] = None, config: VectorSearchConfig = None
    ) -> List[SearchResult]:
        """
        ...
        for node_type in node_types:
            ...
            results = await self._search_node_type(node_type, embedding, "", config)
            all_results.extend(results)
    ...
    def get_search_statistics(self) -> Dict[str, Any]:
        ...
        with self.driver.session() as session:
            for node_type, index_name in self.vector_indexes.items():
                ...
                stats[node_type] = {"nodes_with_embeddings": count, "index_name": index_name}
...
async def main():
    ...
    search_engine = VectorSearchEngine(neo4j_uri="bolt://localhost:7687", neo4j_user="neo4j", neo4j_password="password")

    try:
        # Search for payment-related code
        results = await search_engine.search_by_text(
            "payment processing stripe api", config=VectorSearchConfig(max_results=10, include_raw_code=True)
        )
