...
from .graph_traversal import GraphContext, GraphTraversalEngine, TraversalDirection
from .vector_search import SearchResult, VectorSearchConfig, VectorSearchEngine
...
class QueryParser:
    ...
    def parse_query(self, query: str) -> QueryIntent:
        ...
        for word in words:
            ...
            if (
                word_clean not in self.programming_terms
                and word_clean not in self.node_type_mapping
                and len(word_clean) > 2
            ):
                semantic_terms.append(word_clean)

        # Determine query type and confidence
        query_type, confidence = self._determine_query_type(entity_names, programming_terms, semantic_terms)
    ...
    def _determine_query_type(
        self, entity_names: List[str], programming_terms: List[str], semantic_terms: List[str]
...
class HybridSearchEngine:
    ...
    async def search(self, query: str, config: HybridSearchConfig = None) -> List[HybridSearchResult]:
        """
    ...
    async def _semantic_search(
        self, query: str, intent: QueryIntent, config: HybridSearchConfig
        ...
        vector_results = await self.vector_engine.search_by_text(query, vector_config, node_types)
    ...
    async def _entity_search(self, intent: QueryIntent, config: HybridSearchConfig) -> List[HybridSearchResult]:
    ...
    async def _search_by_name(self, name: str, config: HybridSearchConfig) -> List[SearchResult]:
        ...
        with self.driver.session() as session:
            result = session.run(query, name=name, limit=config.max_entity_results)

            for record in result:
                location = None
                if record["start_line"] and record["end_line"]:
                    location = {"start_line": record["start_line"], "end_line": record["end_line"]}
    ...
    async def _expand_context(
        self, results: List[HybridSearchResult], config: HybridSearchConfig
    ) -> List[HybridSearchResult]:
        ...
        context = await self.graph_engine.expand_context(
            search_results, max_related_nodes=config.max_context_nodes, include_source_code=config.include_source_code
        )
    ...
    async def _add_hierarchical_context(self, result: HybridSearchResult, config: HybridSearchConfig):
        ...
        if config.expand_call_hierarchy and node_type in ["Method", "Function"]:
            call_hierarchy = await self.graph_engine.get_call_hierarchy(
                result.search_result.node_id, direction=TraversalDirection.BOTH, max_depth=2
            )
        ...
        if config.expand_inheritance and node_type in ["Class", "Interface"]:
            inheritance = await self.graph_engine.get_inheritance_hierarchy(result.search_result.node_id)
    ...
    def _merge_and_score_results(
        self, all_results: List[HybridSearchResult], intent: QueryIntent, config: HybridSearchConfig
    ) -> List[HybridSearchResult]:
    ...
    def _calculate_hybrid_score(
        self, result: HybridSearchResult, intent: QueryIntent, config: HybridSearchConfig
    ) -> float:
...
async def main():
    ...
    search_engine = HybridSearchEngine(neo4j_uri="bolt://localhost:7687", neo4j_user="neo4j", neo4j_password="password")
    ...
    try:
        ...
        for query in queries:
            ...
            # Perform the search
            config = HybridSearchConfig(max_total_results=5, enable_context_expansion=True, include_source_code=False)
