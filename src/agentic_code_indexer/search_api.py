...
async def search_code(
    request: SearchRequest, engine: HybridSearchEngine = Depends(get_search_engine)
) -> SearchResponse:
    """
...
async def get_call_hierarchy(
    request: HierarchyRequest, engine: HybridSearchEngine = Depends(get_search_engine)
) -> HierarchyResponse:
    """
    ...
    try:
        ...
        hierarchy = await engine.graph_engine.get_call_hierarchy(
            request.node_id, direction=direction, max_depth=request.max_depth
        )
        ...
        return HierarchyResponse(node_id=request.node_id, hierarchy_type="call", callers=callers, callees=callees)
...
async def get_inheritance_hierarchy(
    ...
    try:
        ...
        return HierarchyResponse(
            node_id=request.node_id, hierarchy_type="inheritance", ancestors=ancestors, descendants=descendants
        )
...
async def get_node_details(node_id: str, engine: HybridSearchEngine = Depends(get_search_engine)):
    """
...
async def get_search_stats(engine: HybridSearchEngine = Depends(get_search_engine)) -> StatsResponse:
    """
...
async def explain_query(
    q: str = Query(..., description="Query to explain"), engine: HybridSearchEngine = Depends(get_search_engine)
):
    """
...
def run_api(host: str = "0.0.0.0", port: int = 8000, reload: bool = False, log_level: str = "info"):
    """
    Run the search API server.
    """
    uvicorn.run(
        "heroes_platform.agentic-code-indexer-standard.src.agentic_code_indexer.search_api:app",
        host=host,
        port=port,
        reload=reload,
        log_level=log_level,
