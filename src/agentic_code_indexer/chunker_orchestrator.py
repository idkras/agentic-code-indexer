...
class ChunkerOrchestrator:
    ...
    def _setup_chunkers(self):
        ...
        # Find the agentic-code-indexer-standard directory
        current_dir = Path(__file__).parent
        agentic_root = current_dir.parent.parent  # Go up to agentic-code-indexer-standard

        # Python chunker
        python_chunker_path = agentic_root / "src" / "python-chunker" / "main.py"
        if python_chunker_path.exists():
            self.chunkers["python"] = ChunkerConfig(
                language="python",
                executable_path=str(python_chunker_path),
                working_directory=str(self.project_root),  # Use project root as working directory
                timeout=300,
            )
            logger.info("Python chunker configured")

        # C# chunker
        csharp_chunker_path = agentic_root / "src" / "csharp-chunker" / "CSharpChunker"
        ...
        # Node.js chunker
        nodejs_chunker_path = agentic_root / "src" / "nodejs-chunker" / "src" / "main.ts"
        if nodejs_chunker_path.exists():
            # Check if compiled JS exists, otherwise use ts-node
            compiled_js = agentic_root / "src" / "nodejs-chunker" / "dist" / "main.js"
    ...
    def get_chunker_for_file(self, file_path: str) -> Optional[ChunkerConfig]:
        ...
        extension_map = {
            ...
            ".mjs": "javascript",
            ".cjs": "javascript",
            ".ts": "javascript",  # Use JavaScript chunker for TypeScript
            ".tsx": "javascript",  # Use JavaScript chunker for TSX
        }
    ...
    async def process_file(self, file_change: FileChange) -> Optional[ChunkerOutput]:
        ...
        try:
            ...
            if chunker_config.language == "python":
                cmd = [
                    "python",
                    chunker_config.executable_path,
            ...
            try:
                stdout, stderr = await asyncio.wait_for(process.communicate(), timeout=chunker_config.timeout)
            ...
            if os.path.exists(temp_output_path):
                ...
                logger.info(
                    f"Successfully processed {file_change.path}: {len(chunker_output.nodes)} nodes, {len(chunker_output.relationships)} relationships"
                )
    ...
    def validate_chunkers(self) -> Dict[str, bool]:
        ...
        for language, config in self.chunkers.items():
            try:
                ...
                result = subprocess.run(cmd, cwd=config.working_directory, capture_output=True, timeout=10)
