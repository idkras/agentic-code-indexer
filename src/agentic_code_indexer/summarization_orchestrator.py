...
class HierarchicalSummarizationOrchestrator:
    ...
    def _create_hierarchical_prompt(self, node: SummarizationNode) -> str:
        ...
        prompt_parts = [f"Analyze and summarize this {node.node_type.lower()}: {node.name}\n"]
    ...
    async def get_nodes_ready_for_processing(
        self, level: SummarizationLevel, batch_size: int = 50
    ) -> List[SummarizationNode]:
