...
from .common_data_format import ChunkerOutput, CodeNode, Relationship
...
class GraphIngestion:
    ...
    def _prepare_node_for_cypher(self, node: CodeNode) -> Dict[str, Any]:
        """Convert a CodeNode to a dictionary suitable for Cypher queries."""
        node_dict = {
            "id": node.id,
            "label": node.label,
            "name": node.name,
    ...
    async def ingest_chunker_output(self, chunker_output: ChunkerOutput) -> IngestionStats:
        ...
        try:
            ...
            logger.info(
                f"Ingestion complete: {stats.nodes_created} nodes created, "
    ...
    async def _ingest_nodes_batch(self, nodes: List[CodeNode]) -> IngestionStats:
    ...
    async def _ingest_relationships_fallback(self, relationships: List[Relationship]) -> IngestionStats:
        ...
        for rel in relationships:
            ...
            rels_by_type[rel_type].append(
                {"source_id": rel.source_id, "target_id": rel.target_id, "properties": rel.properties or {}}
            )
    ...
    async def ingest_multiple_outputs(self, chunker_outputs: List[ChunkerOutput]) -> IngestionStats:
        ...
        logger.info(
            f"Ingesting combined data: {len(all_nodes)} nodes, "
...
async def main():
    ...
    ingestion = GraphIngestion(neo4j_uri="bolt://localhost:7687", neo4j_user="neo4j", neo4j_password="password")
