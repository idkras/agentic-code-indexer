...
from .config import get_config, AgenticCodeIndexerConfig
from .hybrid_search import HybridSearchConfig, HybridSearchEngine
...
def index_command(
    ...
    neo4j_uri: str = typer.Option("bolt://localhost:7688", "--neo4j-uri", help="Neo4j database URI"),
...
def status_command(
    ...
    console.print(
        Panel.fit("[bold cyan]📊 Database Status[/bold cyan]\n" "Current indexing statistics", border_style="cyan")
    )
    ...
    async def run_search():
        ...
        try:
            ...
            for i, result in enumerate(results, 1):
                ...
                row = [str(i), name_text, result.search_result.node_type, score_text, result.match_type]

                if verbose:
                    summary = (
                        result.search_result.summary[:100] + "..."
                        if len(result.search_result.summary) > 100
                        else result.search_result.summary
                    )
                    row.append(summary)
            ...
            if include_code and verbose:
                for i, result in enumerate(results[:3], 1):  # Show code for top 3 results
                    if result.search_result.raw_code:
                        ...
                        console.print(
                            Panel(result.search_result.raw_code, title=result.search_result.name, border_style="dim")
                        )
...
async def run_llm_enrichment(neo4j_uri: str, neo4j_user: str, neo4j_password: str, batch_size: int = 20):
    ...
    try:
        ...
        summary_stats = await summarization_orchestrator.run_hierarchical_summarization(llm_integration, batch_size)
...
@app.callback()
def main(ctx: typer.Context, version: bool = typer.Option(False, "--version", help="Show version information")):
    """
