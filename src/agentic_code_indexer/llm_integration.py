    ...
    async def generate_embedding(self, text: str) -> EmbeddingResult:
        ...
        try:
            ...
            return EmbeddingResult(
                text=text, embedding=embeddings.tolist(), model_name=self.model_name, dimension=len(embeddings)
            )
    ...
    async def generate_embeddings_batch(self, texts: List[str], batch_size: int = 32) -> List[EmbeddingResult]:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                ...
                for text in batch:
                    results.append(EmbeddingResult(text=text, embedding=[], model_name=self.model_name, dimension=0))
...
{node_type.title()}: {code_text}"""
    ...
    async def generate_summary(
        self, text: str, node_type: str = "code", context: Optional[str] = None
    ) -> SummaryResult:
        ...
        try:
            if self.provider == "anthropic":
                ...
                message = await self.client.messages.create(
                    model=self.model, max_tokens=500, temperature=0.1, messages=[{"role": "user", "content": prompt}]
                )
                ...
                return SummaryResult(
                    original_text=text, summary=summary, model_name=self.model, token_count=token_count
                )
    ...
    async def generate_summaries_batch(
        self, texts: List[str], node_types: List[str], contexts: Optional[List[str]] = None, max_concurrent: int = 5
    ) -> List[SummaryResult]:
...
class LLMEmbeddingIntegration:
    ...
    async def run_full_enrichment(
        self, summary_batch_size: int = 50, embedding_batch_size: int = 100
    ) -> Dict[str, int]:
        """Run complete summary and embedding enrichment process."""
        logger.info("Starting LLM and embedding enrichment process")

        stats = {"summaries_generated": 0, "embeddings_generated": 0, "total_iterations": 0}
        ...
        logger.info(
            f"Enrichment complete: {stats['summaries_generated']} summaries, {stats['embeddings_generated']} embeddings"
        )
        return stats
