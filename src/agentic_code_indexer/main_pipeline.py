...
from .file_traversal import FileStatus, FileTraversal
...
class MainPipeline:
    ...
    async def scan_and_detect_changes(self, directory: str) -> List:
        ...
        try:
            file_changes = await self.file_traversal.detect_file_changes(directory, str(self.project_root))
            ...
            for status, count in change_summary.items():
                color = {"new": "green", "modified": "yellow", "deleted": "red", "unchanged": "blue"}.get(
                    status, "white"
                )
...
def run(
    ...
    neo4j_uri: str = typer.Option("bolt://localhost:7688", help="Neo4j URI"),
